...
#include "SuwaMobilitybase.h"
#include <Eigen/Geometry>
registerMooseObject("PhaseFieldApp", SuwaMobilitybase);
template <>
InputParameters
validParams<SuwaMobilitybase>()
{
  ...
  params.addRequiredParam<Real>("Mis_Or","Misorientation of the crystal structure.");
  return params;
}

SuwaMobilitybase::SuwaMobilitybase(const InputParameters & parameters)
  : DerivativeMaterialInterface<Material>(parameters),
    ...
    _thetaRad(0.261799), //15 degrees to radians.
    _Lo(1e-6),
...
SuwaMobilitybase::computeQpProperties()
{
  ...
  Real dM_GBdT;
  _GBMobility = _Lo*(1 - pow(exp(-5 * _Mis/_thetaRad),4));
  if (_GBMobility < 0)
  {
    ...
    dM_GBdT = _M_GB[_qp] * _Q / (_kb * _T[_qp] * _T[_qp]); //If we dont provide a mobility then one temperature dependent
  }                                                        //one is created.
  else
  {
    // Convert to lengthscale^4/(eV*timescale)
  ...
  //std::cout << "Make sure we are here" << " " << _time_scale / (_JtoeV * length_scale4) << std::endl;
  //std::cout << "this one" << " " << dM_GBdT << std::endl;
  // in the length scale of the system

  _l_GB[_qp] = _wGB;

  _L[_qp] = 4.0 / 3.0 * _M_GB[_qp] / _l_GB[_qp];
  //_L[_qp] = _Lo*(1 - exp(pow(((-5*_Mis)/_thetaRad),4)));
  if (_dLdT)
    (*_dLdT)[_qp] = 4.0 / 3.0 * dM_GBdT / _l_GB[_qp];//dM_GBdT is only for temperature dependent GBMobility, zero otherwise
